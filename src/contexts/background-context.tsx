...
interface BackgroundContextType {
  ...
  setBackgroundType: (type: 'none' | 'solid' | 'image' | 'gradient' | 'random') => void
...
export function BackgroundProvider({ children }: { children: ReactNode }) {
  const [background, setBackgroundState] = useState<BackgroundSettings>(defaultBackground)
  ...
  useEffect(() => {
    ...
    if (savedBackground) {
      try {
        ...
        // - If missing value or type is 'none' -> set Lofi Chill Auto video
        // - If legacy type 'random' -> map to 'image' with 'lofi:auto'
        ...
        localStorage.setItem('background-settings', JSON.stringify(defaultBackground))
      }
    } else {
  ...
  const setBackgroundType = (type: 'none' | 'solid' | 'image' | 'gradient' | 'random') => {
    setBackground({
      ...background,
      type,
      // Keep current value unless explicitly switching to none
      value: type === 'none' ? '' : background.value,
    })
  }
